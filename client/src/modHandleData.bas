...
Public Sub InitMessages()
    ...
    HandleDataSub(SPopulateList) = GetAddress(AddressOf HandlePopulateList)
    ' Error handler
    Exit Sub
errorhandler:
...
Private Sub HandlePopulateList(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim i As Long
     ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo errorhandler
    ...
    Exit Sub
errorhandler:
    HandleError "HandlePopulateList", "modHandleData", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
